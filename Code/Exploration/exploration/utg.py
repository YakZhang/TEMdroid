...
class UTG(object):
    ...
    def add_transition(self, event, old_state, new_state):
        ...
        new_state.save_state_xml_for_same_state()
        if old_state.state_str == new_state.state_str:
            # add xml for the current state
            new_state.save_state_xml_for_same_state()
    ...
    def is_state_explored(self, state):
        ...
        if self.device.dfs_graph == 'False':
            ...
            for possible_event in state.get_possible_input_by_category():
                if not self.is_event_explored(possible_event, state):
                    return False
    ...
    def dumpG(self):
        ...
        # read_G1 = nx.read_adjlist("G.adjlist", create_using=nx.DiGraph)
        # read_G2 = nx.read_adjlist("G2.adjlist", create_using=nx.DiGraph)
        """
        nx.write_adjlist(self.G, "G.adjlist")
        nx.write_adjlist(self.G2, "G2.adjlist")
