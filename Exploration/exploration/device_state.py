...
from .input_event import TouchEvent, LongTouchEvent, ScrollEvent, SetTextEvent, KeyEvent, SetTextEnterEvent
import shutil
import random
...
class DeviceState(object):
    ...
    # add static variable
    set_text_id = 0 # static variable for choose corresponding set_text in the set_text_list
    ...
    def save_state_xml(self):
        self.device.adb.shell('uiautomator dump')
        xml_data = self.device.adb.shell('cat /sdcard/window_dump.xml')
        local_xml_path = os.path.join(self.device.output_dir, "states/","xml_%s.xml" % self.tag)
        # self.pull_file('/sdcard/window_dump.xml', local_xml_path)
        # self.save_to_file(xml_data, local_xml_path)
        save_dir = os.path.join(self.device.output_dir, "states/")
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        fh = open(local_xml_path,'w')
        fh.write(xml_data)
        fh.close()

        if self.device.tgt_xml_prefix!=None:

            save_migration_dir = self.device.tgt_xml_prefix
            if os.path.exists(save_migration_dir):
                   if '/Users' in save_migration_dir:
                       if len(save_migration_dir) > 60:
                            # this is make sure the root path cannot be deleted
                            # delete the dir
                            # os.removedirs(save_migration_dir)
                            shutil.rmtree(save_migration_dir)
                   else:
                        shutil.rmtree(save_migration_dir)
            os.mkdir(save_migration_dir)
            fh = open(save_migration_dir+local_xml_path.split("/")[-1],'w')
            fh.write(xml_data)
            fh.close()

    def save_state_xml_for_same_state(self):
        # current state is same as the old state
        self.device.adb.shell('uiautomator dump')
        xml_data = self.device.adb.shell('cat /sdcard/window_dump.xml')
        local_xml_path = os.path.join(self.device.output_dir, "states/", "xml_%s.xml" % self.tag)
        if self.device.tgt_xml_prefix!=None:
    ...
    def __get_view_signature(view_dict):
        ...
        # add content in view_signature
        view_content = DeviceState.__safe_dict_get(view_dict, 'content_description', "None")
        if view_content is None:
            view_content = "None"

        signature = "[class]%s[resource_id]%s[text]%s[content]%s[%s,%s,%s]" % \
    ...
    def get_possible_input(self):
        ...
        for view_id in enabled_view_ids:
            if self.__safe_dict_get(self.views[view_id], 'editable'):
                str_list = ['abc']
                choose_text = random.choice(str_list)
                possible_events.append(SetTextEnterEvent(view=self.views[view_id], text=choose_text))
